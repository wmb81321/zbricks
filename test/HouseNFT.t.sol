...
contract HouseNFTTest is Test {
    ...
    function setUp() public {
        nft = new HouseNFT("Test House", "HOUSE", phaseURIs, owner);
    ...
    function testConstructorInitializesPhaseURIs() public {
        for (uint8 i = 0; i < 4; i++) {
            assertEq(nft.getPhaseURI(i), phaseURIs[i]);
        }
    }
    ...
    function testConstructorRevertsOnZeroAddress() public {
        vm.expectRevert("Invalid initial owner");
        new HouseNFT("Test", "TEST", phaseURIs, address(0));
    }
    
    // ============ Controller Tests ============
    
    function testSetController() public {
        nft.setController(controller);
        
        assertEq(nft.controller(), controller);
    }
    
    function testCannotSetControllerTwice() public {
        nft.setController(controller);
        
        vm.expectRevert("Controller already set");
        nft.setController(address(4));
    }
    
    function testCannotSetControllerToZeroAddress() public {
        vm.expectRevert("Invalid controller");
        nft.setController(address(0));
    }
    ...
    function testCannotSkipPhases() public {
        ...
        nft.advancePhase(2); // Try to skip phase 1
    }
    ...
    function testUpdatePhaseURI() public {
        ...
        nft.updatePhaseURI(0, newURI);
        
        assertEq(nft.getPhaseURI(0), newURI);
    ...
    function testUpdatePhaseURIForFuturePhase() public {
        ...
        // Admin updates URI
        nft.updatePhaseURI(2, newURI);
        
        assertEq(nft.getPhaseURI(2), newURI);
    ...
    function testCannotUpdatePhaseURIWithoutAdmin() public {
        vm.prank(user);
        vm.expectRevert("Only admin");
        nft.updatePhaseURI(0, "ipfs://test");
    }
    ...
    function testCannotUpdateInvalidPhase() public {
        ...
        nft.updatePhaseURI(4, "ipfs://test");
    }
    ...
    function testEmitsEvents() public {
        ...
        emit HouseNFT.ControllerSet(controller);
        nft.setController(controller);
        
        // Test PhaseAdvanced event
        vm.expectEmit(true, true, false, false);
        emit HouseNFT.PhaseAdvanced(1, admin);
        nft.advancePhase(1);
        
        // Test PhaseURIUpdated event (admin updates, not controller)
        vm.expectEmit(true, false, false, true);
        emit HouseNFT.PhaseURIUpdated(0, "ipfs://new");
        nft.updatePhaseURI(0, "ipfs://new");
    }
}
